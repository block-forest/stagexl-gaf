 ...
 */
 part of stagexl_gaf;
	 ...
	 class TAGFXSourcePNGURL extends TAGFXBase
	{
		...
		//
		//--------------------------------------------------------------------------
	 TAGFXSourcePNGURL(String source,Point textureSize,[String format="bgra"]): super() 
		{
		  ...
		  void loadBitmapData(String url)
		{
			...
			{
				try { this._pngLoader.close(); } catch (e) {}
			}
		  ...
		  void onPNGLoadError(IOErrorEvent event)
		{
			...
			LoaderInfo info = event.currentTarget as LoaderInfo;
			throw new StateError("Can't restore lost context from a PNG file. Can't load file: " + info.url, event.errorID);
		}
