...
*/
	 ...
	 * @
	 */
	 class GAFScale9Image extends Sprite implements IValidating, IGAFImage, IMaxSize, IGAFDebug
	{
		//--------------------------------------------------------------------------
		//
		//  PUBLIC VARIABLES
	 ...
	 GAFScale9Image(GAFScale9Texture textures,[num textureScale=1]): super() 
		  ...
		  void validate()
		{
			...
			if (this._propertiesChanged || this._layoutChanged || this._renderingChanged)
			{
				...
				if( sHelperImage == null || sHelperImage == false)
				{
				...
				num scaledRightWidth = (this._frame.width - grid.x - grid.width) * this._textureScale;
				...
				num scaledBottomHeight = (this._frame.height - grid.y - grid.height) * this._textureScale;
