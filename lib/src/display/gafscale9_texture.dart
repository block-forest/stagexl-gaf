 ...
 */
 part of stagexl_gaf;
	 ...
	 class GAFScale9Texture implements IGAFTexture
	{
		 ...
		 * @
		 */
		 static final String DIMENSIONS_ERROR = "The width and height of the scale9Grid must be greater than zero.";
		/**
		 * @
		 */
		...
		//
		//--------------------------------------------------------------------------
	 GAFScale9Texture(String id,Texture texture,Matrix pivotMatrix,Rectangle scale9Grid)
		{
			this._id = id;
		  ...
		  void copyFrom(IGAFTexture newTexture)
		{
			...
			{
				throw new StateError("Incompatiable types GAFScale9Texture and "+getQualifiedTypeName(newTexture));
			}
		}
		//--------------------------------------------------------------------------
		//
		...
		//--------------------------------------------------------------------------
		  ...
		  void initialize()
		{
			Rectangle textureFrame = this._texture.frame;
			if( textureFrame == null || textureFrame == false)
			{
				textureFrame = HELPER_RECTANGLE;
				textureFrame.setTo(0, 0, this._texture.width, this._texture.height);
			}
			num leftWidth = this._scale9Grid.left;
			num centerWidth = this._scale9Grid.width;
			num rightWidth = textureFrame.width - this._scale9Grid.width - this._scale9Grid.x;
			num topHeight = this._scale9Grid.y;
			num middleHeight = this._scale9Grid.height;
			num bottomHeight = textureFrame.height - this._scale9Grid.height - this._scale9Grid.y;

			num regionLeftWidth = leftWidth + textureFrame.x;
			num regionTopHeight = topHeight + textureFrame.y;
			num regionRightWidth = rightWidth - (textureFrame.width - this._texture.width) - textureFrame.x;
			num regionBottomHeight = bottomHeight - (textureFrame.height - this._texture.height) - textureFrame.y;
			...
			Rectangle topLeftFrame = (hasLeftFrame || hasTopFrame) ? new Rectangle(textureFrame.x, textureFrame.y, leftWidth, topHeight) : null;
			this._topLeft = Texture.fromTexture(this._texture, topLeftRegion, topLeftFrame);

			Rectangle topCenterRegion = new Rectangle(regionLeftWidth, 0, centerWidth, regionTopHeight);
			Rectangle topCenterFrame = hasTopFrame ? new Rectangle(0, textureFrame.y, centerWidth, topHeight) : null;
			this._topCenter = Texture.fromTexture(this._texture, topCenterRegion, topCenterFrame);

			Rectangle topRightRegion = new Rectangle(regionLeftWidth + centerWidth, 0, regionRightWidth, regionTopHeight);
			Rectangle topRightFrame = (hasTopFrame || hasRightFrame) ? new Rectangle(0, textureFrame.y, rightWidth, topHeight) : null;
			this._topRight = Texture.fromTexture(this._texture, topRightRegion, topRightFrame);

			Rectangle middleLeftRegion = new Rectangle(0, regionTopHeight, regionLeftWidth, middleHeight);
			Rectangle middleLeftFrame = hasLeftFrame ? new Rectangle(textureFrame.x, 0, leftWidth, middleHeight) : null;
			this._middleLeft = Texture.fromTexture(this._texture, middleLeftRegion, middleLeftFrame);

			Rectangle middleCenterRegion = new Rectangle(regionLeftWidth, regionTopHeight, centerWidth, middleHeight);
			this._middleCenter = Texture.fromTexture(this._texture, middleCenterRegion);

			Rectangle middleRightRegion = new Rectangle(regionLeftWidth + centerWidth, regionTopHeight, regionRightWidth, middleHeight);
			Rectangle middleRightFrame = hasRightFrame ? new Rectangle(0, 0, rightWidth, middleHeight) : null;
			this._middleRight = Texture.fromTexture(this._texture, middleRightRegion, middleRightFrame);

			Rectangle bottomLeftRegion = new Rectangle(0, regionTopHeight + middleHeight, regionLeftWidth, regionBottomHeight);
			Rectangle bottomLeftFrame = (hasLeftFrame || hasBottomFrame) ? new Rectangle(textureFrame.x, 0, leftWidth, bottomHeight) : null;
			this._bottomLeft = Texture.fromTexture(this._texture, bottomLeftRegion, bottomLeftFrame);

			Rectangle bottomCenterRegion = new Rectangle(regionLeftWidth, regionTopHeight + middleHeight, centerWidth, regionBottomHeight);
			Rectangle bottomCenterFrame = hasBottomFrame ? new Rectangle(0, 0, centerWidth, bottomHeight) : null;
			this._bottomCenter = Texture.fromTexture(this._texture, bottomCenterRegion, bottomCenterFrame);

			Rectangle bottomRightRegion = new Rectangle(regionLeftWidth + centerWidth, regionTopHeight + middleHeight, regionRightWidth, regionBottomHeight);
			Rectangle bottomRightFrame = (hasBottomFrame || hasRightFrame) ? new Rectangle(0, 0, rightWidth, bottomHeight) : null;
